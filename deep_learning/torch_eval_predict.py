...
# 6. evaluation
model.eval() # trigger the evaluation mode
test_loss = 0
correct = 0
with torch.no_grad():
    ...
    data, target = data.to(device), target.to(device)
    output = model(data)
    test_loss += F.nll_loss(output, target, reduction='sum').item() # adding sum to return one scala

    pred = output.argmax(dim=1, keepdim=True) # keep the dimension, just reduce the shape
    correct = pred.eq(target.view_as(pred)).sum().item()

print(test_loss)
print(correct)
print(output.shape)
print(pred.shape, target.view_as(pred).shape) # check if both are the same size
